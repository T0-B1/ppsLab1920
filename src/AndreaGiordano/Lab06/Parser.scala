...
trait NotTwoConsecutive[T] extends Parser[T]{
  ...
  abstract override def parse(t: T) =
    if(last.isEmpty || last.getOrElse(AnyRef)!=t){last = Option(t); super.parse(t)} else false
...
object TryParsers extends App {
  ...
  println("----------BasicParser with NotTwoConsecutive with NonEmpty------------")
  ...
  implicit class MyString(base:String){
    def charParser(): Parser[Char] = BasicParser(base.toSet)
  }
  println("----------Parser[Char]------------")
  def sparser : Parser[Char] = "abcqulo".charParser()
  println(sparser.parseAll("aabc".toList)) // true
  println(sparser.parseAll("qulo".toList)) // true
