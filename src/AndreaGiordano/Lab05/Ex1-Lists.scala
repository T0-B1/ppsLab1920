...
trait ListImplementation[A] extends List[A] {
  ...
  override def zipRight: List[(A,Int)] = {
    def myZip(l:List[A],i:Int):List[(A,Int)] = l match {
      case Cons(h,t) => Cons((h,i),myZip(t,i+1))
      case _ => Nil()
    }
    myZip(this,0)
  } // questions: what is the type of keyword ??? = Nothing
  ...
  override def span(pred: A => Boolean): (List[A],List[A]) = {
    @tailrec
    def mySpan(left:List[A],right:List[A]):(List[A],List[A]) = right match {
      case Cons(h,t) if pred(h) => mySpan(left.::(h),t)
      case _ => (left,right)
    }
    mySpan(Nil(),this)
  }
  ...
  override def takeRight(n: Int): List[A] = {
    def _takeRight(l:List[A]):(List[A],Int) = l match {
      case Cons(h,t) => val p=_takeRight(t); if(p._2<n) (Cons(h,p._1),p._2+1) else (p._1,p._2)
      case _ => (Nil(),0)
    }
    _takeRight(this)._1
  }
