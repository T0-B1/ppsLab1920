...
trait NotTwoConsecutive[T] extends Parser[T]{
  private[this] var repetition = false
  private[this] var l: List[T] = List()

  abstract override def parse(t: T) = {
    repetition = repetition || l.contains(t)
    l = l :+ t
    super.parse(t)
  } // who is super??

  abstract override def end() =  {
    l = List()
    !repetition && {repetition = false; super.end()}
  }
}
