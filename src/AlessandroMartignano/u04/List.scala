...
object Lists extends App {
  ...
  object List {
    ...
    def build[A](args: A*): List[A] = {
      var l: List[A] = nil
      for (a <- args)
        l = append(l, Cons(a, nil))
      l
    }
    ...
    @tailrec
    def contains[A](l: List[A])(e: A): Boolean = l match {
    ...
    @tailrec
    def allEqualsTo[E](list: List[E])(e: E): Boolean = list match {
      case Cons(`e`, t) => allEqualsTo(t)(e)
      case Nil() => true
      case _ => false
    }

    def allEquals[E](list: List[E]): scala.Option[E] = list match {
      case Cons(h, t) => if (allEqualsTo(t)(h)) scala.Option(h) else scala.Option.empty
  ...
  val lst2 = Cons(3,Cons(3,Cons(3,Cons(3, Nil()))))
  println(allEquals(lst))
  val lst3 = Cons(3,Cons(3,Cons(3,Cons(4, Nil()))))
  println(allEquals(lst2))
}