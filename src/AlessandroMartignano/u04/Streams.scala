...
object Streams extends App {
  ...
  object Stream {
    ...
    def fromList[A](list: List[A]): Stream[A] = {
      ...
      @tailrec
      def _fromList[A](list: List[A], stream: Stream[A] = empty[A]): Stream[A] = {
        list match {
          case List.Cons(h,t) => _fromList(t, cons(h, stream))
          case _ => stream
        }
      }
      _fromList(l)
  ...
  var l = List.Cons(10, List.Cons(20, List.Cons(30, List.Nil())))
  val s = Stream.fromList(l)
  println(l)
  println(toList(take(s)(5)))
/*
