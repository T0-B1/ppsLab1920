...
trait ListImplementation[A] extends List[A] {
  ...
  override def partition(pred: A => Boolean): (List[A],List[A]) = (filter(pred), filter(e => !pred(e)))

  override def span(pred: A => Boolean): (List[A],List[A]) = {
    def _span(before: List[A])(after: List[A]): (List[A],List[A]) = after match {
      case h :: t if pred(h) =>_span(before.::(h))(t)
      case _ => (before, after)
    }
    _span(Nil())(this)
  }
  ...
  def take(n: Int): List[A] = this match {
    case h :: t if n > 0 => h :: t.asInstanceOf[ListImplementation[A]].take(n-1)
    case _ => Nil()
  }

  override def takeRight(n: Int): List[A] = reverse().asInstanceOf[ListImplementation[A]].take(n)
